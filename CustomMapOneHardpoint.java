...
public class CustomMapOneHardpoint extends JFrame implements KeyListener,MouseMotionListener,MouseListener { 
   ...
   int [] hardpointAreaX = new int[4];
   int [] hardpointAreaY = new int[4];
   ...
   Polygon hardpointArea = new Polygon(hardpointAreaX,hardpointAreaY,4);
   ...
   ImageIcon tank, redTank, blueTank, purpleTank, yellowTank, blueTank2, shot,team, hacked,explosive,laser,lmg,mini,overdrive;
    ...
    /*** CustomMapOneHardpoint ************************************************
    * Purpose: Gets variables set up and sets up images and frame              *                                                                          
    * Parameters: tanks - number of tanks  team1 - team one points stored      *  
    * team2 - team two points stored  team1money - team one money              *
    * team2money - team two money  horf - chose which map                      *
    * playupto - stores how much game goes up to                               *
    * Returns: none                                                            *
    ***************************************************************************/
   public CustomMapOneHardpoint (int tanks, int team1, int team2,int team1money, int team2money, int horf, int playupto) {
      ...
      countdown.scheduleAtFixedRate(countdowns,1005,1000);
      hardpoint.scheduleAtFixedRate(hardpointTimer,100,20000);
   ...
   ***************************************************************************/
   public void seteverything (){
      ...
      hardpointX = (int)(Math.random() * (sizeX))*100+mazex+5;
      hardpointY = (int)(Math.random() * (sizeY))*100+mazey+5;
      hardpointAreaX[0] = hardpointX;
      hardpointAreaX[1] = hardpointX +90;
      hardpointAreaX[2] = hardpointX;
      hardpointAreaX[3] = hardpointX +90;
           
      hardpointAreaY[0] = hardpointY;
      hardpointAreaY[1] = hardpointY;
      hardpointAreaY[2] = hardpointY +90;
      hardpointAreaY[3] = hardpointY +90;
           
      hardpointArea = new Polygon(hardpointAreaX,hardpointAreaY,4);
   ...
   ***************************************************************************/
   ...
   class Drawing extends JComponent {
      ...
      ***************************************************************************/
      public void paint (Graphics g) {
         ...
         Font hardpointTimeNo = new Font("impact",Font.PLAIN,19);
         Font hardpointTime = new Font("impact",Font.BOLD,20);
         g.setFont(hardpointTimeNo);
                 
         g.setColor(Color.cyan);
         g.fillRect(659,1,26,22);
         g.setColor(Color.black);
         g.drawRect(659,1,26,22);
         
        
         g.setColor(Color.black);
         g.drawString("New Hardpoint In: ",515,20);
         g.drawString("Seconds",690,20);
         
         
                     
         if (seconds <10 && seconds >5 )            
            g.drawString("0"+seconds,662,20);
         else if(seconds <=5)
         {
            g.setFont(hardpointTime);
       ...
       // ITEMS
         ...
         if(!cooldownTeam1Done)
         {
            ...
            g.setColor(Color.black);
            g.setFont(title);
            g.drawString("Cooldown period",170,900);
         }
         ...
         if (!cooldownTeam2Done)
         {
            ...
            g.setColor(Color.black);
            g.setFont(title);
            g.drawString("Cooldown period",954+21,900);
         }
         ...
         g.fillRect(hardpointX,hardpointY,90,90);
        
         g.setColor(new Color(255,102,102));
         g.drawRect(hardpointX,hardpointY,90,90);
         ...
         graphics2Dt3.drawRect((t3.posx+t3.width/3),t3.posy,t3.width/3,t3.height/4+10);
         graphics2Dt3.setColor(t3.tankColor);
   ...
   Timer hardpoint =  new Timer();
   TimerTask hardpointTimer =
      new TimerTask() {
      ...
      ***************************************************************************/
         public void run (){
            if(mazedone){
               for (int i =0; i < 8;i++)
               {
                  if(hardpointArea.contains(t1.nx[i], t1.ny[i]))
                  {
                     t1IsInside = true;
                  }
               }
               ...
               for (int i =0; i < 8;i++)
               {
                  if(hardpointArea.contains(t2.nx[i], t2.ny[i]))
                  {
                     t2IsInside = true;
                  }
               ...
               for (int i =0; i < 8;i++)
               {
                  if(hardpointArea.contains(t3.nx[i], t3.ny[i]))
                  {
                     t3IsInside = true;
                  }
               ...
               for (int i =0; i < 8;i++)
               {
                  if(hardpointArea.contains(t4.nx[i], t4.ny[i]))
                  {
                     t4IsInside = true;
                  }
               }
  ...
  ***************************************************************************/
   ...
   ***************************************************************************/
      ...
      class Keys extends Thread {
         ...
         /*** run *******************************************************************
      * Purpose: Tracks the movement of the bullets for the time its active      *                                                                          
      * Parameters: none                                                         *                                   
      * Returns: none                                                            *
      ***************************************************************************/
         public void run() {
  ...
  // Clicking Powerups
   ...
   public void mouseReleased(MouseEvent e)
   {
      ...
      if (clickedI7 && notDisabledRight && team1Money >= 600 && t1powerup && t4powerup && cooldownTeam1Done && t1powerup && t4powerup && (t1.isVisible || t4.isVisible))
